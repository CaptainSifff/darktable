...
void dt_print_file(const int32_t imgid, const char *filename, const dt_print_info_t *pinfo)
{
  ...
  dt_get_image_layout (imgid, pinfo, 0, 0, &ax, &ay, &awidth, &aheight, &ix, &iy, &iwidth, &iheight);
...
void dt_get_image_layout(const int32_t imgid, const dt_print_info_t *prt,
                         ...
                         int32_t *ix, int32_t *iy, int32_t *iwidth, int32_t *iheight)
{
  ...
  // display page
  int32_t p_x, p_y, p_bottom, p_right;

  if (a_aspect > pg_aspect)
  {
    p_x = (width - (height * pg_aspect)) / 2;
    p_y = 0;
    p_bottom = height;
    p_right = width - p_x;
  }
  ...
  {
    p_x = 0;
    p_y = (height - (width / pg_aspect)) / 2;
    p_right = width;
    p_bottom = height - p_y;
  }

  const int32_t p_width = p_right - p_x;
  const int32_t p_height = p_bottom - p_y;
  ...
  const int32_t bx = p_x + ((np_left + border_left) / pg_width) * p_width;
  const int32_t by = p_y + ((np_top + border_top)/ pg_height) * p_height;
  const int32_t bb = p_bottom - ((np_bottom + border_bottom) / pg_height) * p_height;
  const int32_t br = p_right - ((np_right + border_right) / pg_width) * p_width;
