...
_print_button_clicked (GtkWidget *widget, gpointer user_data)
{
  ...
  // compute the needed size for picture
  ...
  snprintf(filename, PATH_MAX, "/tmp/pf_%d", imgid);
...
_printer_profile_changed(GtkComboBox *widget, dt_lib_module_t *self)
{
  ...
  dt_conf_set_string("plugins/print/printer/iccprofile", "image");
}
...
gui_init (dt_lib_module_t *self)
{
  ...
  GtkWidget *label4 = gtk_label_new(_("alignement"));
  ...
  gtk_combo_box_set_active(GTK_COMBO_BOX(d->profile), 0);
  ...
  gtk_combo_box_set_active(GTK_COMBO_BOX(d->style), 0);
  ...
  while(l)
  {
    ...
    if (!strcmp(prof->filename,"sRGB") && !strcmp(prof->filename,"adobergb"))
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(d->pprofile), prof->name);
    l = g_list_next(l);
  }

  gtk_combo_box_set_active(d->pprofile, 0);
  snprintf(tooltip, sizeof(tooltip), _("output ICC profiles in %s/color/out or %s/color/out"), confdir, datadir);
  g_object_set(G_OBJECT(d->pprofile), "tooltip-text", tooltip, (char *)NULL);
  gtk_combo_box_set_active(GTK_COMBO_BOX(d->pprofile), 0);
  ...
  // gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(d->pintent), _("image settings"));
