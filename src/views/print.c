...
static void expose_print_page(dt_view_t *self, cairo_t *cr, int32_t width, int32_t height, int32_t pointerx, int32_t pointery)
{
  ...
  double a_aspect = (double)width / (double)height;
  ...
  cairo_rectangle (cr, p_x, p_y, p_width, p_height);
  cairo_fill (cr);
  ...
  // top-left
  int npx = p_x + (np_left / pg_width) * p_width;
  int npy = p_y + (np_top / pg_height) * p_height;
  cairo_move_to (cr, npx, npy); cairo_line_to (cr, npx-10, npy);
  cairo_move_to (cr, npx, npy); cairo_line_to (cr, npx, npy-10);
  cairo_stroke (cr);

  // top-right
  npx = p_right - (np_right / pg_width) * p_width;
  ...
  // bottom-left
  npx = p_x + (np_left / pg_width) * p_width;
  ...
  dt_get_image_layout (prt->image_id, prt->pinfo, width, height, &ax, &ay, &awidth, &aheight, &ix, &iy, &iwidth, &iheight);
